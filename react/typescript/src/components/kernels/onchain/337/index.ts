...
const krnlProvider = new ethers.JsonRpcProvider(import.meta.env.VITE_RPC_KRNL);
 ...
 */
export async function executeKrnl(address?: string, customKernelId?: string): Promise<any> {
 ...
 */
export async function callContractProtectedFunction(executeResult: any, signer: any): Promise<string> {
    if (!signer) {
    ...
    const contract = new ethers.Contract(CONTRACT_ADDRESS as string, contractAbi, signer);
