...
// KRNL Response interface (can be expanded based on actual response structure)
...
function App() {
  ...
  const [networkSwitchPending, setNetworkSwitchPending] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (CONTRACT_ADDRESS && provider) {
      ...
      contract.on('Broadcast', (sender: string, data: ethers.BigNumberish, message: string, event: ethers.EventLog) => {
  ...
  const executeKrnlWithWallet = async (): Promise<void> => {
    try {
      setLoading(true);
  ...
  const getIcon = (): string => {
    return NETWORKS[selectedNetwork]?.icon || './sepolia.svg';
  };

  // Get network name
  const getNetworkName = (): string => {
    return NETWORKS[selectedNetwork]?.chainName || 'Sepolia';
  };
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center">
              <h1 className="text-xl font-bold flex items-center gap-2">
                <img src="/logo.svg" width={32} height={32} alt="Logo" className="inline-block" />
                <span className="text-white">KRNL</span>
                <span className="text-[#001EFE]">React Template</span>
              </h1>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                ...
                >
                  <div className="mr-2">
                    <img src={getIcon()} width={24} height={24} alt="Network Icon" className="inline-block" />
                  </div>
                    ...
                    {Object.entries(NETWORKS).map(([networkName, network]) => (
                      ...
                      >
                        <div className="flex-shrink-0 w-5 h-5">
                          <img src={network.icon} width={20} height={20} alt={`${network.chainName} Icon`} className="inline-block" />
                        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-300 max-w-2xl mx-auto">
            Interact with your registered smart contract using this React template.
        ...
        {response && (
          <div className="bg-gray-900 rounded-xl shadow-2xl border border-gray-800 mb-8">
            <div className="bg-black px-6 py-4 border-b border-gray-800">
              <h3 className="text-xl font-semibold">Kernel Responses</h3>
            </div>
            <div className="p-6">
              {responseLoading ? (
